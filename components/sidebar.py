...
def render_sidebar(default_refresh_secs: int = 5) -> dict:
    with st.sidebar:
        ...
        # Account size (use key 'acct' to match main app expectations)
        acct = st.number_input("Account size ($)", min_value=50.0, value=100.0, step=50.0, key="acct")

        # One risk slider only; allow a one-time downshift override
        default_risk = st.session_state.get("next_session_risk_pct", 25.0) or 25.0
        risk_pct = st.slider("Risk per trade (%)", 1, 50, int(default_risk), step=1, key="risk_pct")
        ...
        if "next_session_risk_pct" in st.session_state and st.session_state.next_session_risk_pct:
            # apply only on first render of the new session; add date logic if desired
        ...
        max_risk = float(acct) * (float(risk_pct) / 100.0)
        max_contracts = max(1, int(max_risk // float(est_cost))) if est_cost > 0 else 1
    ...
    # Return keys that the main app actually uses
    return {
        "enable_refresh": enable_refresh,
        "freeze_charts": freeze_charts,
